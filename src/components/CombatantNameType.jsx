...
const CombatantNameType = ({ name, type, onUpdate, isMobile }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [editedName, setEditedName] = useState(name || 'New Combatant');
  ...
  useEffect(() => {
    setEditedName(name || 'New Combatant');
    setEditedType(type);
  }, [name, type]);
  ...
  const handleBlur = () => {
    if (!isSelectOpen) {
      setIsEditing(false);
      const finalName = editedName.trim() || 'New Combatant';
...
export default CombatantNameType;