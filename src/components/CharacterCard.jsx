...
const CharacterCard = ({ character, updateCharacter, removeCharacter, isActive, turnTime, onPreviousTurn, onNextTurn, setIsNumericInputActive, onInitiativeBlur, onInitiativeSubmit }) => {
  ...
  const getBorderStyle = () => {
    return isActive ? 'border-[3px] border-white dark:border-gray-300' : 'border border-black dark:border-gray-700';
  };

  const getTabColor = () => {
    return isActive ? 'bg-black text-white' : 'bg-white text-black dark:bg-black dark:text-white';
  };
  ...
  const getToggleButtonStyle = (isActive) => {
    return `h-[30px] px-2 text-xs border ${isActive ? 'bg-black text-white dark:bg-gray-200 dark:text-black' : 'bg-white text-black dark:bg-gray-800 dark:text-white'} transition-colors`;
  };
      ...
      <div className={`w-16 flex-shrink-0 ${getTabColor()} flex flex-col items-center justify-between py-2`}>
        <div className="flex flex-col items-center">
          ...
          <Input
            ...
            className="w-12 text-center bg-white text-black dark:bg-black dark:text-white h-[30px]"
            maxLength={3}
          />
        </div>
        {isActive && (
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center space-x-2">
              <Input
                ...
                className="w-16 text-center bg-white text-black dark:bg-black dark:text-white h-[30px]"
                placeholder="Current"
                maxLength={3}
              />
              ...
              <div className="flex items-center">
                <Input
                  ...
                  onFocus={() => setIsNumericInputActive(true)}
                  onBlur={() => setIsNumericInputActive(false)}
                  className="w-16 text-center h-[30px] bg-white text-black dark:bg-black dark:text-white"
                  placeholder="Max"
                  maxLength={3}
                />
          ...
          <div className="mt-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                variant="outline"
                className="h-[30px] px-3 py-1 text-sm flex items-center dark:border-gray-600 dark:text-white"
              >
        ...
        <div className="mt-auto self-end">
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button 
                variant="link" 
                className="btn-sm text-gray-700 hover:text-red-500 transition-colors duration-200 whitespace-nowrap dark:text-gray-400 dark:hover:text-red-400"
              >
                Delete character
              </Button>
            </AlertDialogTrigger>
