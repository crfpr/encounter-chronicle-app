...
const CharacterCard = ({ character, updateCharacter, removeCharacter, isActive, turnTime, onPreviousTurn, onNextTurn, setIsNumericInputActive, onInitiativeBlur, onInitiativeSubmit }) => {
  ...
  const getBorderStyle = () => {
    return isActive ? 'border-[3px]' : 'border';
  };
  ...
  return (
    <div className={`flex bg-white dark:bg-black relative overflow-hidden rounded-lg ${getBorderStyle()} border-black dark:border-white box-content transition-all duration-200 ease-in-out`} style={{ height: '240px' }}>
      {/* Left Tab */}
      <div className={`w-16 flex-shrink-0 ${getTabColor()} ${isActive ? '' : 'border-r border-black dark:border-white'} flex flex-col items-center justify-between py-2 transition-colors duration-200`}>
      ...
      {/* Right Tab */}
      <div className={`w-24 flex-shrink-0 ${getTabColor()} ${isActive ? '' : 'border-l border-black dark:border-white'} flex flex-col items-center justify-between p-2 transition-colors duration-200`}>
