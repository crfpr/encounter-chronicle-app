...
const CharacterCard = ({ character, updateCharacter, removeCharacter, isActive, turnTime, onPreviousTurn, onNextTurn, setIsNumericInputActive, onInitiativeBlur, onInitiativeSubmit, isMobile, round }) => {
  ...
  const getBorderStyle = () => isActive ? 'border-zinc-800 dark:border-zinc-800' : 'border-zinc-300 dark:border-zinc-800';
  const getTabColor = () => isActive ? 'bg-zinc-800 text-white dark:bg-zinc-800 dark:text-zinc-100' : 'bg-white text-black dark:bg-zinc-950 dark:text-zinc-100';
  ...
  const handleTokenChange = (tokenId, updates) => {
      ...
      <div className="flex-grow p-2 flex flex-col">
        <div className="flex-grow space-y-2">
          <div className="flex items-start space-x-2 relative">
            <div className="flex-grow flex items-start">
              ...
              <CharacterStats
                ac={character.ac}
                onAcChange={(value) => handleInputChange('ac', value)}
                onInputKeyDown={handleNumericInputKeyDown}
          ...
          <div className="flex items-center flex-wrap gap-2">
            {tokens.map((token) => (
              <Badge
                key={token.id}
                className={`h-[30px] px-1 flex items-center space-x-1 group ${
                  isActive
              ...
              >
                <TokenInput
                  ...
                  onToggleDuration={() => handleTokenChange(token.id, { showDuration: !token.showDuration })}
                />
                <Button
        ...
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <Button 
              variant="link" 
              className="btn-sm text-xs text-zinc-700 dark:text-zinc-300 hover:text-red-500 dark:hover:text-red-400 transition-colors duration-200 whitespace-nowrap bg-transparent dark:bg-transparent p-0 h-auto flex items-end self-end mt-auto"
            >
              Delete
            </Button>
          </AlertDialogTrigger>
