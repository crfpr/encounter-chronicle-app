...
const ConditionInput = ({ condition, onLabelChange, onDurationChange, onRemove, onTogglePersistent }) => {
  ...
  const [localLabel, setLocalLabel] = useState(condition.label);
  const labelInputRef = useRef(null);
  const durationInputRef = useRef(null);

  const [localDuration, handleDurationChange, handleDurationKeyDown, setLocalDuration] = useNumericInput(condition.conditionDuration, 0);
  ...
  useEffect(() => {
    setLocalDuration(condition.conditionDuration);
  }, [condition.conditionDuration, setLocalDuration]);
  ...
  return (
    <div className="flex items-center space-x-1">
      ...
      {isDurationEditing || !condition.isPersistent ? (
        <Input
